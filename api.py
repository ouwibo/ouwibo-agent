...
async def list_skills(_: Any = Depends(require_auth)) -> dict[str, Any]:
    ...
    return {
        "count": len(skills),
        "skills": [
            {"id": s.slug, "title": s.manifest.name, "description": s.manifest.description}
            for s in skills
        ],
    }
...
async def get_skill(skill_id: str, _: Any = Depends(require_auth)) -> dict[str, Any]:
    ...
    return {"id": s.slug, "title": s.manifest.name, "description": s.manifest.description, "content": s.content}
...
async def get_agent_json():
    return FileResponse(os.path.join(os.path.dirname(__file__), "static", ".well-known", "agent.json"))


@app.get("/.well-known/agent-card.json", tags=["Metadata"])
async def get_agent_card_json():
    return FileResponse(os.path.join(os.path.dirname(__file__), "static", ".well-known", "agent-card.json"))
