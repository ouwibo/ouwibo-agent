...
export default function SimpleAgentChat() {
  ...
  return (
    <div className="flex h-screen bg-transparent text-foreground font-sans selection:bg-[#ff4d4d]/30 relative z-10">
      
      {/* MOBILE HEADER */}
      <header className="md:hidden h-14 border-b border-white/5 flex items-center justify-between px-4 bg-[#050810]/80 backdrop-blur-md absolute top-0 w-full z-30">
        <button onClick={() => setIsSidebarOpen(true)} className="p-2 text-zinc-400">
          <Menu size={20} />
        </button>
        <div className="font-bold text-sm flex items-center gap-2">
          <div className="w-6 h-6 bg-[#ff4d4d] rounded flex items-center justify-center text-white text-[10px]">O</div>
          Ouwibo Agent
        </div>
        <div className="w-8" />
      </header>
        ...
        {(isSidebarOpen || (typeof window !== 'undefined' && window.innerWidth >= 768)) && (
          <>
            ...
            <motion.aside 
              ...
              className="fixed md:static inset-y-0 left-0 w-64 border-r border-[#1f2937] bg-[#050810]/80 backdrop-blur-xl flex flex-col z-40"
            >
              <div className="p-4 border-b border-[#1f2937] flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 bg-[#ff4d4d] rounded-lg flex items-center justify-center text-white font-black shadow-[0_0_15px_rgba(255,77,77,0.3)] text-xs">O</div>
              ...
              <div className="p-4">
                ...
                >
                  <Plus size={16} className="text-[#ff4d4d]" />
      ...
      {/* MAIN CHAT AREA */}
      <main className="flex-1 flex flex-col relative w-full h-full pt-14 md:pt-0 bg-transparent">
        
        {/* Desktop Header */}
        <header className="hidden md:flex h-14 border-b border-[#1f2937] items-center justify-between px-6 bg-[#050810]/50 backdrop-blur-md absolute top-0 w-full z-10">
          <div className="flex items-center gap-3 text-xs text-zinc-400 font-medium">
            <span className="flex items-center gap-2 text-[#00e5cc]">
              <div className="w-2 h-2 bg-[#00e5cc] rounded-full animate-pulse shadow-[0_0_8px_#00e5cc]" />
              Neural Link Active
            </span>
          </div>
          <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-[#1f2937] text-[10px] mono text-zinc-400 glass-card">
            <Cpu size={12} className="text-[#00e5cc]" />
            Auto-Routing
          </div>
        </header>
          ...
          <div className="max-w-3xl mx-auto space-y-8 pb-32">
            {messages.length === 1 && messages[0].provider === 'System' && (
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col items-center justify-center h-64 mt-12 opacity-80">
                <div className="w-20 h-20 bg-[#ff4d4d]/5 border border-[#ff4d4d]/20 rounded-3xl flex items-center justify-center text-5xl mb-6 shadow-[0_0_30px_rgba(255,77,77,0.15)]">🦍</div>
            ...
            {isLoading && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="flex gap-4 lg:gap-6 justify-start">
                <div className="w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-xl border bg-[#ff4d4d]/10 border-[#ff4d4d]/20 shadow-[0_0_15px_rgba(255,77,77,0.05)]">🦍</div>
                <div className="flex items-center gap-2 p-4 rounded-2xl bg-white/[0.02] border border-white/5 text-zinc-400 text-sm">
                  <div className="w-1.5 h-1.5 bg-[#ff4d4d] rounded-full animate-bounce" style={{ animationDelay: '0ms' }} />
                  <div className="w-1.5 h-1.5 bg-[#ff4d4d] rounded-full animate-bounce" style={{ animationDelay: '150ms' }} />
                  <div className="w-1.5 h-1.5 bg-[#ff4d4d] rounded-full animate-bounce" style={{ animationDelay: '300ms' }} />
        ...
        {/* Input Area */}
        <div className="absolute bottom-0 left-0 right-0 p-4 lg:p-8 bg-gradient-to-t from-[#050810] via-[#050810]/90 to-transparent z-20">
          <div className="max-w-3xl mx-auto relative">
            <div className="relative group">
              <div className="absolute -inset-1 bg-gradient-to-r from-[#ff4d4d]/10 via-[#00e5cc]/10 to-transparent rounded-2xl blur opacity-0 group-focus-within:opacity-100 transition-opacity duration-500" />
              <div className="relative flex items-end bg-[#0a0f1a]/90 backdrop-blur-2xl border border-[#1f2937] rounded-2xl p-2 pl-4 focus-within:border-[#ff4d4d]/50 transition-all shadow-2xl">
                <textarea 
                  value={input}
                ...
                <button 
                  ...
                  className="ml-2 bg-[#ff4d4d] hover:bg-[#ff4d4d]/90 disabled:opacity-30 disabled:hover:bg-[#ff4d4d] text-white p-3 rounded-xl transition-all active:scale-95 flex items-center justify-center mb-1 shadow-[0_0_15px_rgba(255,77,77,0.2)] disabled:shadow-none"
                >
                  <Send size={18} />
                </button>
              </div>
...
function MessageBubble({ role, content, provider, time }: any) {
  ...
  return (
    ...
    >
      {isAgent && (
        <div className="w-8 h-8 md:w-10 md:h-10 rounded-xl flex items-center justify-center flex-shrink-0 text-base md:text-xl border bg-[#ff4d4d]/10 border-[#ff4d4d]/20 shadow-[0_0_15px_rgba(255,77,77,0.05)]">
          🦍
        </div>
      )}
      ...
      <div className={`flex flex-col space-y-1 max-w-[90%] lg:max-w-[80%] ${!isAgent ? 'items-end' : 'items-start'}`}>
        ...
        <div className={`p-4 md:p-5 rounded-2xl text-[14.5px] leading-relaxed w-full overflow-x-auto ${
          isAgent 
            ? 'bg-white/[0.02] border border-white/[0.05] text-zinc-200 shadow-sm rounded-tl-sm glass-card' 
            : 'bg-[#1a202c] border border-white/5 text-zinc-100 rounded-tr-sm shadow-md'
        }`}>
          {isAgent ? (
            <div className="prose prose-invert max-w-none prose-sm md:prose-base prose-p:leading-relaxed prose-pre:bg-[#050810] prose-pre:border prose-pre:border-white/10 prose-code:text-[#00e5cc]">
